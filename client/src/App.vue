...
async function confirm(suggestion) { loading.value = true; error.value = ''; try { const studentIds = teacher.value.students.filter((s) => suggestion.studentNames.includes(s.name)).map((s) => s.id); await api.teacher.createLesson({ studentIds, startAt: suggestion.startAt }); teacherScheduleDate.value = suggestion.startAt; teacher.value.suggestion = null; teacher.value.draft = ''; notice.value = '预约已确认，并已加入课表。'; await loadTeacher(); } catch (e) { error.value = `预约未保存（${e.message}）。请检查可用课时后重试。`; } finally { loading.value = false; } }
